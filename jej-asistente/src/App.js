...
const USUARIOS = [
  { correo: 'operario@jej.cl',   rut: '123456789', nombre: 'Carlos Ramírez' },
  { correo: 'supervisor@jej.cl', rut: '987654321', nombre: 'Ana Torres' },
...
function LoginScreen({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    const cleanRut = r => r.replace(/[^0-9kK]/gi, '').toUpperCase();
    const user = USUARIOS.find(
      u => u.correo.toLowerCase() === correo.toLowerCase().trim() &&
           cleanRut(u.rut) === cleanRut(rut)
    );
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          <div style={styles.field}>
            ...
            <input type="email" value={correo} onChange={e => setCorreo(e.target.value)}
              placeholder="nombre@jej.cl" required style={styles.input} />
          </div>
          <div style={styles.field}>
            <label style={styles.label}>RUT</label>
            <input type="text" value={rut} onChange={handleRut}
              placeholder="12.345.678-9" maxLength={12} required style={styles.input} />
          </div>
...
const reply = data.response || data.output || data.text || data.message || (typeof data === 'string' ? data : JSON.stringify(data)) || 'No pude obtener una respuesta.';
    ...
    <div style={styles.chatWrap}>
      ...
      {messages.length === 1 && (
        <div style={styles.chips}>
          {sugerencias.map((s, i) => (
            <button key={i} style={styles.chip} onClick={() => { setInput(s); inputRef.current?.focus(); }}>{s}</button>
          ))}
        </div>
      )}
      <div style={styles.inputBar}>
        <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
          onKeyDown={handleKey} placeholder="Escribe tu consulta..." rows={1} style={styles.textarea} />
        <button onClick={send} disabled={!input.trim() || loading} style={{
          ...styles.sendBtn, background: input.trim() && !loading ? C.navy : C.gray300,
        }}>
...
const styles = {
  loginWrap: { minHeight: '100vh', background: '#F7F9FC', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: "'DM Sans', sans-serif" },
  loginCard: { width: '100%', maxWidth: 420, borderRadius: 20, overflow: 'hidden', boxShadow: '0 8px 40px rgba(27,63,122,0.15)' },
  loginHeader: { background: 'linear-gradient(135deg, #122d5a 0%, #2a5298 100%)', padding: '36px 32px 28px', textAlign: 'center' },
  ...
  form:   { background: '#FFFFFF', padding: '28px 32px 24px' },
  field:  { marginBottom: 20 },
  label:  { display: 'block', fontSize: 13, fontWeight: 500, color: '#7A8899', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' },
  input:  { width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid #C4CDD8', fontSize: 15, color: '#2C3E50', outline: 'none', boxSizing: 'border-box', fontFamily: "'DM Sans', sans-serif" },
  errorBox: { background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '10px 14px', color: '#D64545', fontSize: 13, marginBottom: 16 },
  btn: { width: '100%', padding: '14px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #1B3F7A 0%, #2a5298 100%)', color: '#FFFFFF', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  loginFooter: { background: '#EEF1F6', textAlign: 'center', padding: '14px', margin: 0, fontSize: 12, color: '#7A8899' },
  chatWrap: { display: 'flex', flexDirection: 'column', height: '100vh', background: '#F7F9FC', fontFamily: "'DM Sans', sans-serif", position: 'relative' },
  navbar: { background: 'linear-gradient(135deg, #122d5a 0%, #2a5298 100%)', padding: '12px 16px', display: 'flex', alignItems: 'center', boxShadow: '0 2px 12px rgba(27,63,122,0.25)', position: 'relative', zIndex: 10 },
  menuBtn: { background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px' },
  dropdown: { position: 'absolute', top: '100%', right: 12, background: '#FFFFFF', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.12)', minWidth: 200, overflow: 'hidden', zIndex: 100 },
  dropItem: { padding: '12px 16px', fontSize: 14, color: '#2C3E50' },
  messages: { flex: 1, overflowY: 'auto', padding: '16px 12px 8px' },
  avatar:   { width: 32, height: 32, borderRadius: '50%', marginRight: 8, flexShrink: 0, background: 'linear-gradient(135deg, #1B3F7A, #4A90D9)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  bubble: { padding: '10px 14px', fontSize: 14 },
  typingDots: { display: 'flex', gap: 4, padding: '2px 4px' },
  dot: { width: 7, height: 7, borderRadius: '50%', background: '#C4CDD8', display: 'inline-block', animation: 'bounce 1.2s ease-in-out infinite' },
  chips: { padding: '0 12px 8px', display: 'flex', flexDirection: 'column', gap: 8 },
  chip: { background: '#E8F1FB', border: '1px solid #4A90D9', borderRadius: 20, padding: '8px 14px', fontSize: 13, color: '#1B3F7A', cursor: 'pointer', textAlign: 'left', fontFamily: "'DM Sans', sans-serif" },
  inputBar: { display: 'flex', alignItems: 'flex-end', gap: 8, padding: '10px 12px 16px', background: '#FFFFFF', borderTop: '1px solid #EEF1F6' },
  textarea: { flex: 1, padding: '10px 14px', borderRadius: 20, border: '1.5px solid #C4CDD8', fontSize: 14, resize: 'none', fontFamily: "'DM Sans', sans-serif", outline: 'none', background: '#F7F9FC', color: '#2C3E50', lineHeight: 1.5, maxHeight: 100, overflowY: 'auto' },
  sendBtn: { width: 42, height: 42, borderRadius: '50%', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 },
};
